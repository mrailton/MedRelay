...
def test_events_index(client, db_session, organisation):
...
def test_events_index_requires_auth(client, db_session, organisation):
...
def test_events_show(client, db_session, organisation):
    _login(client, db_session, organisation)
    event = create_event(db_session, name="Detail Event", organisation=organisation)
    response = client.get(f"/events/{event.id}")
    assert response.status_code == 200
    assert "Detail Event" in response.text


def test_events_show_404(client, db_session, organisation):
    _login(client, db_session, organisation)
    response = client.get("/events/9999", follow_redirects=False)
    assert response.status_code == 303


def test_events_edit(client, db_session, organisation):
    _login(client, db_session, organisation)
...
def test_events_update(client, db_session, organisation):
    _login(client, db_session, organisation)
    event = create_event(db_session, name="Original", organisation=organisation)
    csrf = _csrf(client)
    response = client.post(
        f"/events/{event.id}",
        data={
