...
def test_incidents_index(client, db_session, organisation):
...
def test_incidents_store(client, db_session, organisation):
    _login(client, db_session, organisation)
    event = create_event(db_session, organisation=organisation)
    csrf = _csrf(client)
...
def test_incidents_show_404(client, db_session, organisation):
    _login(client, db_session, organisation)
    response = client.get("/incidents/9999", follow_redirects=False)
    assert response.status_code == 303


def test_incidents_update_status(client, db_session, organisation):
    _login(client, db_session, organisation)
    event = create_event(db_session, organisation=organisation)
    incident = make_incident(db_session, event)
    resource = create_resource(db_session, event, {"name": "Unit-1", "resource_type": "AMBULANCE"}, _login(client, db_session, organisation))
    db_session.commit()
...
def test_incidents_assign_resource(client, db_session, organisation):
    _login(client, db_session, organisation)
    event = create_event(db_session, organisation=organisation)
    incident = make_incident(db_session, event)
...
def test_incidents_add_note(client, db_session, organisation):
    _login(client, db_session, organisation)
    event = create_event(db_session, organisation=organisation)
    incident = make_incident(db_session, event)
    csrf = _csrf(client)
...
def test_add_incident_note_service(db_session, organisation):
    event = create_event(db_session, organisation=organisation)
...
def test_get_incident_service(db_session, organisation):
    event = create_event(db_session, organisation=organisation)
    incident = make_incident(db_session, event)
...
def test_get_incident_with_details_service(db_session, organisation):
    event = create_event(db_session, organisation=organisation)
    incident = make_incident(db_session, event)
...
def test_list_incidents_by_event(db_session, organisation):
    event = create_event(db_session, organisation=organisation)
...
def test_count_incidents_by_event(db_session, organisation):
    event = create_event(db_session, organisation=organisation)
...
def test_count_active_incidents_by_event(db_session, organisation):
    event = create_event(db_session, organisation=organisation)
